...
def call_cnn_model():
    ...
    input_shape = 1, 61, 61 if backend.image_data_format() == 'channels_first' else 61, 61, 1
...
def train_cnn(dataset, dirname):
    ...
    Inputs:
        ...
        dirname : The directory into which their results will be put
        datafile: The training/test splits to be used. If not, it uses default one.
                  This data file is an object with attributes related to training, test and validation splits
                  (see  create_folds function and Data_Fold class in utils.py)
        lighter_cnn: If True, this has less number of hidden units in the top layers of the CNN
                     (see construct_cnn_model)
    Outputs: Saves the best model and the average precision scores for baselines and CNN for each fold
    """
    # Set number of of lds and runs for each fold
    num_folds = 10
    num_runs = 5
    ...
    for i in range(num_folds):
        ...
        for run in range(num_runs):
            ...
            # Print the number of samples in each class after data augmentation
            print('Labels-1 : {:d}, labels-0 : {:d}'.format(int(tr_output.sum()), int((tr_output == 0).sum())))

            # Callbacks
            early_stop = EarlyStopping(monitor='val_loss', min_delta=0, patience=5, verbose=1, mode='auto')
            model_name = os.path.join(dirname, 'best_model_fold_{:d}_run_{:d}.h5'.format(i, run))
            ...
            result_sum = 'Fold {:d}: MD: {:.3f} -- SLV: {:.3f} -- MD+SLV: {:.3f} -- CNN: {:.3f}\n'.format(
                i, precs[i, run, 0], precs[i, run, 1], precs[i, run, 2], precs[i, run, 3]
            )
            # Write the results to the summary file
            with open(os.path.join(dirname, 'result_summary.txt'), mode='a') as f:
                f.write(result_sum)
        ...
        with open(os.path.join(dirname, 'result_summary.txt'.format(dirname)), mode='a') as f:
...
def train_nn(dataset, dirname, datafile=None, lighter_nn=False):
    ...
    # Save the results into npz file
    np.savez(os.path.join(dirname, 'ap_scores.npz'), precs=precs)
    ...
    plt.boxplot(precs.reshape(num_runs * num_folds, 4), labels=fig_x_labels)
    plt.grid()
    plt.savefig(os.path.join(dirname, 'ap_performance'.format(dirname)))
    # plt.show()
    plt.close()
