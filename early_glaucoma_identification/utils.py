...
def create_data(dataset):
    ...
    if dataset == 'BD':
        ...
        raise NotImplementedError('No dataset is available with that name!')

    df = pd.read_csv(os.path.join('S1_data', '{}_dataset.csv'.format(dataset)))
    ...
    val_pat_ids = []
    test_pat_ids = []
    with open(os.path.join('S1_data', '{}_training_patient_ids.csv'.format(dataset)), 'r') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
            tr_pat_ids.append(np.asarray(row).astype('int'))

    with open(os.path.join('S1_data', '{}_validation_patient_ids.csv'.format(dataset)), 'r') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
            val_pat_ids.append(np.asarray(row).astype('int'))

    with open(os.path.join('S1_data', '{}_test_patient_ids.csv'.format(dataset)), 'r') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
...
def generate_voronoi_images_given_image_size(data, xy_coordinates, image_size=(61, 61)):
    """
    Generates Voronoi images using given values for patching colors and given coordinates for seed points.
    Here image size is fixed or given by the user.
    Output conventions according to Theano CNN implementation
      (4D tensor -- (number_of_samples, number_of_channels, image_row, image_col))
    Inputs: 
    ...
    Outputs:
        vor_images:
          NxMxRxC voronoi images,
          4D tensor -- (number of samples N, number of channels M, number of rows R, number_of_columns C)
    """
    ...
    # Fill in image
    n_channels = 1
    vor_images = np.zeros((num_obs, n_channels, img_row_size, img_col_size))
